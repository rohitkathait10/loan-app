@extends('layouts.guest')

@section('content')

<div class="customer-login-bg">
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-md-8 col-lg-6 col-xl-5">
                <div class="card shadow-lg customer-login-card">
                    <div class="card-body px-5 py-4">

                        <div class="d-flex justify-content-center mb-3">
                            <img src="{{ asset('admin/assets/img/logo-new.png') }}" alt="Logo" class="login-img-fluid">
                        </div>

                        <h2 class="text-center fw-bold mb-4" style="font-size: 1.8rem;">Reset Password</h2>

                        <form method="POST" action="{{ route('password.mobile') }}">
                            @csrf

                            <div class="mb-3">
                                <label class="form-label">Enter Registered Mobile Number</label>
                                <input type="text" name="phone"
                                    value="{{ old('phone') }}"
                                    class="form-control @error('phone') is-invalid @enderror"
                                    required placeholder="Mobile Number (10 digits)">

                                @error('phone')
                                    <span class="text-danger small">{{ $message }}</span>
                                @enderror
                            </div>

                            <div class="d-grid">
                                <button type="submit" class="btn customer-login-btn text-white">Send OTP</button>
                            </div>

                            <div class="text-center mt-4">
                                Already have an account?
                                <a href="{{ route('login') }}" class="text-apply">Login Now</a>
                            </div>

                        </form>

                    </div>
                </div>
            </div>
        </div>
    </div>
</div>

@endsection
